-- Update the referral reward processing function to work with correct plan IDs
...
BEGIN
  ...
  IF NEW.plan_id IN ('basic-monthly', 'monthly') AND OLD IS NULL THEN
...
-- Update the handle_new_user function to process referrals from signup data
...
DECLARE
  ...
  referrer_id UUID;
BEGIN
  -- Insert profile data from auth user metadata
  INSERT INTO public.profiles (id, first_name, last_name, full_name, phone_number)
  VALUES (
    NEW.id, 
  ...
  referral_email := NEW.raw_user_meta_data ->> 'referral_email';
  IF referral_email IS NOT NULL AND referral_email != '' THEN
    -- Find the referrer by email
    SELECT au.id INTO referrer_id
    FROM auth.users au
    WHERE au.email = referral_email;
...
-- Update the get_effective_user_plan function to handle the new plan IDs
