-- Fix the referral reward trigger to use started_at instead of created_at
...
BEGIN
  ...
  IF NEW.plan_id IN ('basic', 'premium') AND OLD IS NULL THEN
    -- Get the month-year for this purchase using started_at instead of created_at
    current_month_year := TO_CHAR(NEW.started_at, 'YYYY-MM');
    ...
    IF referrer_id IS NOT NULL THEN
      ...
      SET first_paid_purchase_at = NEW.started_at 
      WHERE referred_user_id = NEW.user_id;
